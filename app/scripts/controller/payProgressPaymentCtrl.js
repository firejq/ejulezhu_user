...
angular.module('app').controller('payProgressPaymentCtrl', ['$http', '$scope', '$state', 'cache', function ($http, $scope, $state, cache) {
	...
	$http({
		...
		if (response.data.Status === 0) {
			...
			$scope.payProgressPaymentData = response.data;

			//格式化下单时间，原格式：20171014092816，格式化为2017-10-14 09:28:16
			var time = $scope.payProgressPaymentData.OrderTime;
			$scope.payProgressPaymentData.OrderTime = time.substring(0, 4) + '-' + time.substring(4, 6) + '-' + time.substring(6, 8) + ' ' + time.substring(8, 10) + ':' + time.substring(10, 12) + ':' + time.substring(12, 14);
			time = $scope.payProgressPaymentData.Appointmenttime;
			$scope.payProgressPaymentData.Appointmenttime = time.substring(0, 4) + '-' + time.substring(4, 6) + '-' + time.substring(6, 8) + ' ' + time.substring(8, 10) + ':' + time.substring(10, 12) + ':' + time.substring(12, 14);
